...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if ($current_password !== $hp['staffPassword']) {
        echo "<script>alert('Current password is incorrect.');</script>";
    } elseif ($new_password !== $confirm_password) {
        echo "<script>alert('New password and confirm password do not match.');</script>";
    } else {
        // Update password in the database
        $update_query = "UPDATE staff SET staffPassword = '$new_password' WHERE staffID = '$staffid'";
        
        if (mysqli_query($condb, $update_query)) {
            echo "<script>alert('Password updated successfully.'); window.location.href = 'staff_profile.php';</script>";
        } else {
